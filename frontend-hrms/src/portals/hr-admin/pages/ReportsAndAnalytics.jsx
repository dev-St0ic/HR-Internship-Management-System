...
export default function ReportsAndAnalytics() {
  ...
  const totalIncoming = useMemo(
    () => internshipOverview.reduce((sum, item) => sum + item.incoming, 0),
    [internshipOverview],
  );
  const totalActive = useMemo(
    () => internshipOverview.reduce((sum, item) => sum + item.active, 0),
    [internshipOverview],
  );
  const totalFinalizing = useMemo(
    () => internshipOverview.reduce((sum, item) => sum + item.finalizing, 0),
    [internshipOverview],
  );
            ...
            <div className="p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className={`mt-4 text-3xl font-semibold ${metric.text}`}>
                    {metric.value}
                  </p>
                </div>
                <div
                  className={`flex h-12 w-12 items-center justify-center rounded-3xl ${metric.accent}`}
                >
          ...
          <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Incoming, active, and finalizing internships this quarter
              </p>
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
              <div className="flex items-end justify-between gap-4 h-[340px] px-3">
                {internshipOverview.map((item) => {
                  const total =
                    item.incoming + item.active + item.finalizing || 1;
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div className="flex flex-col gap-5 lg:flex-row lg:items-center">
                  <div className="flex h-44 w-44 items-center justify-center rounded-full bg-slate-100">
                    <div className="flex h-36 w-36 items-center justify-center rounded-full bg-white text-3xl font-semibold text-slate-900">
                      155
                    </div>
                  </div>
                  ...
                  <svg viewBox="0 0 360 220" className="h-full w-full">
                    ...
                    {chartDays.map((series, idx) => {
                      ...
                      return (
                        <g key={series.week}>
                          <path
                            d={`${pathData} L 360 200 L 0 200 Z`}
